...
void SynthesisSimplified(int t, int nene)
{
	
//TeX output file
ofstream totex;
totex.open("MC_Synthesis_Simplified_TeX.txt");
...
//string RecoInd[NReco]={"OneIterForwardPRUniB","OneIterForwardPRNoDummy","OneIterForwardPR1mmDummy","TwoIterForwardPRUniB","TwoIterForwardPRNoDummy","TwoIterForwardPR1mmDummy"};
const int NReco=5;
//string RecoInd[NReco]={"OneIterForwardPRUniB","OneIterForwardPRNoDummy","OneIterForwardPR1mmDummy"};
string RecoInd[NReco]={"OneIterForwardPRUniB","OneIterForwardPRNoDummy","OneIterForwardPR1mmDummy","TwoIterForwardPRUniB","TwoIterForwardPR1mmDummy"};

//Number of configurations to study
const int NConfig=1;
string TConfig[NConfig]={"T1&T4&reco"};					   
...
\begin{table}[H]
\centering
\caption{OneIterForwardUniB} \label{tab:title} \\ 

 \begin{tabular}{|c|c|c||}
  \hline
   Energy (in MeV) & fit $\mu$/ $\sigma$  & resolution & $\chi$^2/ndf \\
   \hline
    20&0.005/0.00001&12\% &302/6 \\
    40&0.005/0.003&34\% &452/78 \\
    \hline
 \end{tabular}\par
 \bigskip
 This is a caption 
...
for(int i=0; i<NReco; i++) 
	{
	fileout[i]=new TFile(Form("%s/%d/KF/Synthesis_Simplified%s_%d_%s.root", directory.c_str(),type,startfile.c_str(), type,RecoInd[i].c_str()),"RECREATE");
  ...
  // 		cout << "Reco Type: " << RecoInd[i] << "  Energy: " << Ene[j] << UNIT <<endl;
		...
		for(int k=0;k<Ncycles[j];k++)//Number of cycles
			{
        		chain[i][j]->Add(Form("%s/%d/RecoEvent_%s_%d_%d%s%03d%s_%s.root",Inppath.c_str(),type,startfile.c_str(),type,Ene[j],UNIT.c_str(),k+1,endfile.c_str(),RecoInd[i].c_str()));
//cout << Form("Add file RecoEvent_%s_%d_%d%s%03d%s_%s.root",startfile.c_str(),type,Ene[j],UNIT.c_str(),k+1,endfile.c_str(),RecoInd[i].c_str()) <<"  to chain " << endl;
			} // end k
...
<<"&"<<fgauss[i][j][k][0]->GetChisquare()<<"/"<<fgauss[i][j][k][0]->GetNDF()<<" //" <<endl;
	...
	pMC[i][j][k]->Write();
     ...
     can[i][j]->Write();
     can[i][j]->SaveAs(Form("%s/%d/KF/Reconstructions_Simplified_%s_%d%s.eps", directory.c_str(),type,RecoInd[i].c_str(),Ene[j],UNIT.c_str()));
    if (i==0&&j==0) can[i][j]->Print(Form("%s/%d/KF/Reconstructions_Simplified.pdf(", directory.c_str(),type));
    else if (i==NReco-1&&j==Nene-1) can[i][j]->Print(Form("%s/%d/KF/Reconstructions_Simplified.pdf)",directory.c_str(),type));
    else can[i][j]->Print(Form("%s/%d/KF/Reconstructions_Simplified.pdf", directory.c_str(),type));
  } //end j
	
	totex <<" \hline" <<endl;
	totex <<"end{tabular}\par"
...
////////////////////////////////////
 ...
 for(int i =0; i<NReco;i++) { 
    ...
    canchi2[i][j]->SaveAs(Form("%s/%d/KF/Chi2_Simplified_%s_%d%s.eps", directory.c_str(),type,RecoInd[i].c_str(),Ene[j],UNIT.c_str()));
    if (i==0&&j==0) canchi2[i][j]->Print(Form("%s/%d/KF/Chi2_Simplified.pdf(", directory.c_str(),type));
    else if (i==NReco-1&&j==Nene-1) canchi2[i][j]->Print(Form("%s/%d/KF/Chi2_Simplified.pdf)",directory.c_str(),type));
    else canchi2[i][j]->Print(Form("%s/%d/KF/Chi2_Simplified.pdf", directory.c_str(),type));
  } //end j
} //end i
...
for(int i=0;i<NReco;i++) {
    ...
    CanMomPulls[i]->SaveAs(Form("%s/%d/KF/MomReso_Simplified_%s.eps", directory.c_str(),type,RecoInd[i].c_str()));
    if (i==0) CanMomPulls[i]->Print(Form("%s/%d/KF/MomReso_Simplified.pdf(", directory.c_str(),type));
    else if (i==NReco-1) CanMomPulls[i]->Print(Form("%s/%d/KF/MomReso_Simplified.pdf)",directory.c_str(),type));
    else CanMomPulls[i]->Print(Form("%s/%d/KF/MomReso_Simplified.pdf", directory.c_str(),type));
} //end i
