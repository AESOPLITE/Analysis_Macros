...
void DisplayEventsDefault(string file,string coinc,int geoconf)
{
 ...
 for(int i=0;i<Nevents;i++)
   ...
   // if(i>505000)continue;
    ...
    //if(e->get_chi2NB()<=0||e->get_chi2NB()>1) continue;
    //if(e->get_chi2B()<=0||e->get_chi2B()>1) continue;
    ...
    for(int ijk=0;ijk<7;ijk++)
      {
       Line[ijk]=new TLine(-9,zL[ijk],9,zL[ijk]);
...
//	if(e->get_eventnumber()==33418) can->Print(Form("EVTDISPLAY_%s_evt%d_poscandidate.pdf",file.c_str(),(int)e->get_eventnumber()));
	//if(e->get_eventnumber()==20284) can->Print(Form("EVTDISPLAY_%s_evt%d_elecandidate.pdf",file.c_str(),(int)e->get_eventnumber()));
	//if(e->get_eventnumber()==9286) can->Print(Form("EVTDISPLAY_%s_evt%d_muplusprotoncandidate.pdf",file.c_str(),(int)e->get_eventnumber()));
//	if(e->get_eventnumber()==31250) can->Print(Form("EVTDISPLAY_%s_evt%d_muminuscandidate.pdf",file.c_str(),(int)e->get_eventnumber()));
...
void DisplayEvent(string file,string tag,string coinc,int geoconf,int EVTNum)
{
 ...
 for(int i=0;i<Nevents;i++)
   ...
   // if(e->get_chi2B()<=0||e->get_chi2B()<10) continue;
    ...
    for(int ijk=0;ijk<7;ijk++)
      {
       TNh[ijk]=new TPaveText(-20,zL[ijk]-2,-18,zL[ijk]+2);
    ...
    for(int i=0;i<nPHA;i++)
      {
       ...
       if(Ztrig[i]>zL[6])//Need to use incoming line
         {
